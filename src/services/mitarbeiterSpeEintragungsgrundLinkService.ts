...
export const mitarbeiterSpeEintragungsgrundLinkService = {
  getAll: async (): Promise<MitarbeiterSpeEintragungsgrundLink[]> => {
    return new Promise((resolve) => {
      setTimeout(
        () => resolve(data as MitarbeiterSpeEintragungsgrundLink[]),
        100
      );
    });
  },

  getBySchichtmodell: async (
    schichtmodellId: number
  ): Promise<MitarbeiterSpeEintragungsgrundLink[]> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const items = data.filter(
          (item) => item.schichtmodellId === schichtmodellId
        );
