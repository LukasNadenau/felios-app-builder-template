...
export const mitarbeiterRessourceBedarfLinkService = {
  ...
  getByMitarbeiterAndArbeitsgang: async (
    mitarbeiterId: number,
    arbeitsgangId: number
  ): Promise<MitarbeiterRessourceBedarfLink[]> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const items = data.filter(
          (item) =>
            item.mitarbeiterId === mitarbeiterId &&
            item.arbeitsgangId === arbeitsgangId
        );
  ...
  getByArbeitsgang: async (
    arbeitsgangId: number
  ): Promise<MitarbeiterRessourceBedarfLink[]> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const items = data.filter(
          (item) => item.arbeitsgangId === arbeitsgangId
        );
        resolve(items as MitarbeiterRessourceBedarfLink[]);
      }, 100);
    });
